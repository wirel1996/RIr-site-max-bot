...
sheet = Roo::Spreadsheet.open(path).sheet(4)
...
def norm_address(value)
  norm(value)
    .gsub(/\bул\b|\bулица\b|\bг\b|\bсеверск\b/, ' ')
    .gsub(/\s+/, ' ')
    .strip
end
...
def build_attrs(sheet, row)
  ...
  return nil unless uuid?(identifier) || [name, address, phone, email].any? { |value| !value.to_s.strip.empty? }
...
def find_existing(db, attrs)
  ...
  unless identifier.empty?
    row = db.get_first_row(
      "SELECT * FROM contacts WHERE category = 'embedded' AND identifier = ? ORDER BY id LIMIT 1",
      [identifier]
    )
  ...
  unless address_key.empty?
    db.execute("SELECT * FROM contacts WHERE category = 'embedded' AND address IS NOT NULL ORDER BY id").each do |row|
  ...
  row = db.get_first_row(
    "SELECT * FROM contacts WHERE category = 'embedded' AND lower_ru(coalesce(name, '')) = ? ORDER BY id LIMIT 1",
...
ContactsDB.with_db do |db|
  ...
  begin
    ...
    ContactsDB.set_meta(db, 'last_embedded_contacts_import_at', now)
    db.execute('COMMIT')
