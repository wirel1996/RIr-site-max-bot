...
export default function MeteringGspo() {
  ...
  const importMutation = useMutation({
    ...
    onError: (err: { message?: string }) => setMessage(err.message || t.gspo.importFailed),
  })
  ...
  return (
    <div className="space-y-4">
      <nav className="text-sm"><Link to="/metering" className="text-blue-600 hover:underline">{t.gspo.back}</Link></nav>
      ...
      {message && (
        <div className="rounded border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span>{message}</span>
            {importDetails.length > 0 && <button type="button" onClick={() => setImportDetailsOpen(true)} className="rounded border border-blue-300 bg-white px-2 py-1 text-xs text-blue-700 hover:bg-blue-50">{t.gspo.viewChanges}</button>}
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-5xl rounded-lg bg-white p-4 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-base font-semibold text-gray-900">{t.gspo.changesTitle}</h2>
              <button type="button" onClick={() => setImportDetailsOpen(false)} className="rounded border bg-white px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100">{t.gspo.close}</button>
            </div>
            <div className="max-h-[70vh] overflow-auto rounded border border-gray-200">
              <table className="min-w-full text-left text-xs text-gray-800">
                <thead className="bg-gray-50"><tr><th className="px-2 py-1">{t.gspo.row}</th><th className="px-2 py-1">{t.gspo.object}</th><th className="px-2 py-1">{t.gspo.changes}</th></tr></thead>
                <tbody>
                  {importDetails.map((row, index) => (
                    <tr key={`${row.id}-${index}`} className="border-t">
                      <td className="px-2 py-1">{row.source_row}</td>
                      <td className="px-2 py-1"><Link to={`/metering/gspo/${row.id}`} className="text-blue-700 hover:underline">{row.name || row.address || row.identifier || `#${row.id}`}</Link></td>
      ...
      {isLoading && <div className="text-gray-500">{t.gspo.loading}</div>}
      {error && <div className="rounded border border-red-200 bg-red-50 p-3 text-red-800">{t.gspo.loadFailed}</div>}
      {data && data.records.length === 0 && <p className="rounded bg-white p-4 shadow text-gray-500">{t.gspo.nothingFound}</p>}

      {data && data.records.length > 0 && (
        <div className="overflow-x-auto rounded-lg bg-white shadow">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100 text-left"><tr><th className="px-3 py-2 w-14">#</th><th className="px-3 py-2">{t.gspo.table.name}</th><th className="px-3 py-2">{t.gspo.table.address}</th><th className="px-3 py-2">{t.gspo.table.calculator}</th><th className="px-3 py-2">{t.gspo.table.nearest}</th></tr></thead>
            <tbody>
              {data.records.map((record, index) => (
                <tr key={record.id} className="border-t hover:bg-gray-50">
                  <td className="px-3 py-2 text-gray-500">{data.page * data.page_size + index + 1}</td>
                  <td className="px-3 py-2 max-w-80"><Link to={`/metering/gspo/${record.id}`} className="block break-words text-blue-700 hover:underline">{record.name || '—'}</Link></td>
      ...
      {data && pageCount > 1 && (
        <div className="flex items-center justify-between">
          <button type="button" disabled={page === 0} onClick={() => setPage(page - 1)} className="rounded border bg-white px-3 py-1.5 text-sm disabled:opacity-40 hover:bg-gray-100">{t.gspo.prev}</button>
          <span className="text-sm text-gray-600">{t.gspo.page(page + 1, pageCount)}</span>
          <button type="button" disabled={page >= pageCount - 1} onClick={() => setPage(page + 1)} className="rounded border bg-white px-3 py-1.5 text-sm disabled:opacity-40 hover:bg-gray-100">{t.gspo.next}</button>
        </div>
      )}
    </div>
  )
